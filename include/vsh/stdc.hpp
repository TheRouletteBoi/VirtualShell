...
static int fprintf(FILE* stream, const char* fmt, ...)
{
    ...
    va_list args;
    va_start(args, fmt);
    r = stdc::vfprintf(stream, fmt, args);
    va_end(args);
    return r;
}
...
static const char* va(const char* format, ...)
{
    va_list args;
    va_start(args, format);
    vsnprintf(vaBuffer, 0x400, format, args);
    va_end(args);
    return vaBuffer;
}
...
static const wchar_t* wva(const wchar_t* format, ...)
{
    va_list args;
    va_start(args, format);
    vswprintf(wvaBuffer, 0x400, format, args);
    va_end(args);
    return wvaBuffer;
}
...
#define vsh_assert(test)	((test) ? (void)0 \
    : stdc::stdc_DDC71A75(__FILE__ ":" _STRIZE(__LINE__) " " #test, _FUNNAME))
#else /* __CELL_ASSERT__ */
#define vsh_assert(test)	((test) ? (void)0 \
    : stdc::stdc_AF89FDBD(__FILE__ ":" _STRIZE(__LINE__) " " #test, _FUNNAME))
